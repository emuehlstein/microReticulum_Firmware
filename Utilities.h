...
extern RNS::Reticulum reticulum;
  ...
  void boot_seq() { }
    ...
    void led_tx_off() { digitalWrite(pin_led_tx, HIGH); }
		...
		// LEDs are active HIGH on the ThinkNode M6
		void led_rx_on()  { digitalWrite(pin_led_rx, HIGH); }
		void led_rx_off() {	digitalWrite(pin_led_rx, LOW); }
		void led_tx_on()  { digitalWrite(pin_led_tx, HIGH); }
		void led_tx_off() { digitalWrite(pin_led_tx, LOW); }
...
bool eeprom_product_valid() {
	...
	#elif PLATFORM == PLATFORM_NRF52
	if (rval == PRODUCT_RAK4631 || rval == PRODUCT_HELTEC_T114 || rval == PRODUCT_TECHO || rval == PRODUCT_THINKNODE_M6 || rval == PRODUCT_HMBRW) {
...
bool eeprom_model_valid() {
  ...
  #elif BOARD_MODEL == BOARD_THINKNODE_M6
  if (model == MODEL_M6_US || model == MODEL_M6_EU) {
	#elif BOARD_MODEL == BOARD_HUZZAH32
	if (model == MODEL_FF) {
	#elif BOARD_MODEL == BOARD_GENERIC_ESP32
	if (model == MODEL_FF || model == MODEL_FE) {
